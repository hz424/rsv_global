...
# Output files (now in results/ folder)
RESULTS_DIR = Path("results")
RESULTS_DIR.mkdir(exist_ok=True)
OUTPUT_FREQUENCY_TABLE = RESULTS_DIR / "rsv_F_mutation_frequency_by_country_mafft_extracted_final.tsv"
...
def load_public_metadata(file_path, required_cols):
    try:
        ...
        df = pd.read_csv(file_path, sep='\t', usecols=required_cols, on_bad_lines='warn',
                         low_memory=False, dtype=dtype_spec)
        print(f"Successfully loaded {len(df)} records from {file_path}", file=sys.stderr)
        return df
    except FileNotFoundError:
        print(f"ERROR: Metadata file not found: {file_path}", file=sys.stderr)
...
def find_protein_mutations(sample_protein_seq, ref_protein_seq):
    ...
    for i in range(min_len):
        ...
        if ref_aa in {'-', 'X', 'B', 'Z', 'J', '*'} or sample_aa in {'-', 'X', 'B', 'Z', 'J', '*'}:
            continue
        if ref_aa != sample_aa:
            mutations.add(f"{ref_aa}{i+1}{sample_aa}")
    return mutations
    ...
    with tempfile.NamedTemporaryFile(mode="w+", delete=True, suffix=".fasta") as tmp_in:
        ...
        try:
            result = subprocess.run(
                mafft_cmd, capture_output=True, text=True, check=False,
                timeout=MAFFT_TIMEOUT_SECONDS, stdin=subprocess.DEVNULL
            )
            if result.returncode == 0 and result.stdout:
                aligned = {rec.id: str(rec.seq) for rec in SeqIO.parse(io.StringIO(result.stdout), "fasta")}
                if ref_cds_seq_record.id in aligned and public_seq_id_for_mafft in aligned:
                    aref = aligned[ref_cds_seq_record.id]
                    apub = aligned[public_seq_id_for_mafft]
                    if len(aref) != len(apub):
                        return None
                    L = len(aref)
                    s, e = -1, -1
                    for i in range(L):
                    ...
                    if s != -1:
                        seg = apub[s:e+1]
                        if is_rc_input:
                            extracted_f_gene_cds = str(Seq(seg.replace("-", "")).reverse_complement())
                        else:
                            extracted_f_gene_cds = seg.replace("-", "")
        except subprocess.TimeoutExpired:
            return None
        except FileNotFoundError:
            print("CRITICAL ERROR: MAFFT command not found. Ensure MAFFT is installed and in PATH.", file=sys.stderr)
            return None
        except Exception:
            return None
...
def process_public_sequence(args_tuple):
    ...
    if extracted_fwd and len(extracted_fwd) >= min_expected_f_gene_cds_len_abs:
        ...
        # Reverse complement fallback
        public_seq_rc_str = str(Seq(full_nt_public_str).reverse_complement())
...
def main():
    ...
    try:
        ...
        print(f"CRITICAL ERROR: Unexpected error during MAFFT check: {type(e).__name__}: {e}", file=sys.stderr)
        traceback.print_exc()
        sys.exit(1)
    ...
    try:
        ...
        print(f"Loaded UAE mutation matrix: {uae_matrix_full_df.shape[0]} mutations, {uae_matrix_full_df.shape[1]} samples", file=sys.stderr)
    except FileNotFoundError:
    ...
    uae_assignment_df = load_uae_metadata(
        UAE_ASSIGNMENT_FILE,
        ['Sample', 'AssignedType', 'MappedReads_A', 'MappedReads_B', 'ReferenceLength', 'MeanDepth', 'CoveredBases', 'CoverageFraction']
    )
    ...
    print(f"Retained {len(uae_assignment_filtered_df)} / {initial_uae_samples} UAE samples after filtering.", file=sys.stderr)
    ...
    if not valid_filtered_uae_samples:
    ...
    if not ref_a_nt_map or not ref_b_nt_map:
        sys.exit(1)
    ...
    if not pub_a_meta_df.empty:
        ...
        pub_a_meta_recent_df['country_clean'] = pub_a_meta_recent_df['country'].apply(extract_country_simple) if not pub_a_meta_recent_df.empty else pd.Series(dtype='str')
    ...
    if not pub_b_meta_df.empty:
        ...
        pub_b_meta_recent_df['country_clean'] = pub_b_meta_recent_df['country'].apply(extract_country_simple) if not pub_b_meta_recent_df.empty else pd.Series(dtype='str')
        ...
        for accession_id in pub_a_meta_recent_unique['accession']:
            if accession_id not in already_parsed_a_ids and accession_id in pub_a_full_seqs_nt:
                ...
                tasks_a.append((accession_id, pub_a_full_seqs_nt[accession_id], country, 'A',
                                ref_a_cds_seq_record, ref_a_prot,
                                min_expected_f_gene_cds_len_a_abs, min_expected_f_prot_len_a_abs))
        ...
        for accession_id in pub_b_meta_recent_unique['accession']:
            if accession_id not in already_parsed_b_ids and accession_id in pub_b_full_seqs_nt:
                ...
                tasks_b.append((accession_id, pub_b_full_seqs_nt[accession_id], country, 'B',
                                ref_b_cds_seq_record, ref_b_prot,
                                min_expected_f_gene_cds_len_b_abs, min_expected_f_prot_len_b_abs))
    ...
    for reason, count_val in skipped_stats.items():
        print(f"  Skipped {count_val} NEW public sequences due to: {reason}", file=sys.stderr)
    ...
    if total_successfully_processed_public > 0:
        ...
        print(f"Unique known countries assigned: {unique_countries_assigned}", file=sys.stderr)
    ...
    if not uae_matrix_df.empty:
        for mutation, row in uae_matrix_df.iterrows():
            ...
            if mutation_type == 'A' and total_uae_a_filtered > 0:
                uae_freq_list.append({
                    'Mutation': mutation, 'Type': 'A', 'Country': 'UAE',
                    'Frequency': count / total_uae_a_filtered, 'Count': int(count),
                    'Total_Samples': total_uae_a_filtered
                })
            elif mutation_type == 'B' and total_uae_b_filtered > 0:
                uae_freq_list.append({
                    'Mutation': mutation, 'Type': 'B', 'Country': 'UAE',
                    'Frequency': count / total_uae_b_filtered, 'Count': int(count),
                    'Total_Samples': total_uae_b_filtered
                })
        ...
        for country_val, type_dict in country_type_samples.items():
            for s_type_val, samples_in_group in type_dict.items():
                ...
                for mut_str, count_val in mutations_in_group_counts.items():
                    pub_freq_list.append({
                        'Mutation': mut_str, 'Type': s_type_val, 'Country': country_val,
                        'Frequency': count_val / total_samples_in_group, 'Count': count_val,
                        'Total_Samples': total_samples_in_group
                    })
